...
.modal {
    ...
    width: 100%; /* 팝업 너비를 100%로 설정하여 화면 크기에 맞게 크기 조절 */
...
<body>
	...
	<div class="page-main">
		...
		<div class="search-form">
			<h2>음식 검색</h2>
			<form
				action="${pageContext.request.contextPath}/mydiet/noteDietSearch.do"
				method="get">
			...
			<c:if test="${not empty foodList}">
				<ul>
					<c:forEach var="food" items="${foodList}">
						<li>${food.foodName}<!-- 추가 버튼 추가 -->
							<button type="button" class="add-button"
								data-food-id="${food.dietId}" data-food-name="${food.foodName}">추가</button>
						</li>
					</c:forEach>
				</ul>
	...
	<div id="food-modal" class="modal">
		<div class="modal-content">
			...
			<form id="add-meal-form">
				<input type="hidden" id="food-id" name="foodId"> <label
					for="meal-type">식사 유형:</label><br> <input type="radio"
					id="breakfast" name="mealType" value="Breakfast"> <label
					for="breakfast">아침</label><br> <input type="radio" id="lunch"
					name="mealType" value="Lunch"> <label for="lunch">점심</label><br>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.addEventListener('click', function(event) {
            if (event.target === document.getElementById('food-modal')) {
                document.getElementById('food-modal').style.display = 'none';
            }
        });
        ...
        document.getElementById('add-meal-form').addEventListener('submit', function(event) {
            ...
            // 서버로 전송 (AJAX)
            fetch('/mydiet/addMealLog.do', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ foodId, mealType })
            })
            .then(response => response.json())
            .then(data => {
                // 서버에서 처리 후 알림
                alert('식사가 추가되었습니다!');
                document.getElementById('food-modal').style.display = 'none';  // 팝업 닫기
            })
            .catch(error => {
