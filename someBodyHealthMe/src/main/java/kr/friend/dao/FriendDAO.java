...
public class FriendDAO {
	...
	private FriendDAO() {}
	   private Connection conn;
	    ...
	    public List<Integer> getReceivedFriendRequests(int receiverNum) {
	        ...
	        public boolean sendFriendRequest(int senderNum, int receiverNum) {
	            boolean isRequestSent = false;

	            String sql = "INSERT INTO friend (friend_num, sender_num, receiver_num, status, created_at) " +
	                         "VALUES (seq_friend.nextval, ?, ?, '1', SYSDATE)";  // '1'은 친구 요청 상태
    ...
    public boolean addFriend(int senderNum, int receiverNum, String status) {
        ...
        String sql = "INSERT INTO friend (friend_num, user_num, status, created_at) " +
    ...
    public FriendVO getMember(long mem_num)throws Exception{
		...
		try {
			...
			if(rs.next()) {
				...
				friend.setPassword(rs.getString(rs.getString("password")));
				...
				friend.setModify_Date(rs.getDate("smodify_date"));
