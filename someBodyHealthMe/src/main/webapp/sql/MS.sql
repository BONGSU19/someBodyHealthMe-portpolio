    ...
    user_num NUMBER NOT NULL,                    -- 유저 번호 (Foreign Key)
    status VARCHAR2(10) NOT NULL,                 -- 친구 수락 상태 표시 ('요청1', '수락2', '거절3')
    CONSTRAINT friend_pk PRIMARY KEY (friend_num),  -- 친구번호를 기본키로 설정
    CONSTRAINT friend_user_fk FOREIGN KEY (user_num) REFERENCES SUSER(user_num)  -- 유저 번호에 대한 외래키 설정
);
...
CREATE TABLE Messages (
    ...
    message_date DATE NOT NULL,                  -- 메시지 시간
    CONSTRAINT messages_pk PRIMARY KEY (message_num),  -- 메시지 아이디를 기본키로 설정
    CONSTRAINT messages_sender_fk FOREIGN KEY (sender_num) REFERENCES USER(user_num),  -- 발신자 아이디 외래키
    CONSTRAINT messages_receiver_fk FOREIGN KEY (receiver_num) REFERENCES USER(user_num)  -- 수신자 아이디 외래키
);
