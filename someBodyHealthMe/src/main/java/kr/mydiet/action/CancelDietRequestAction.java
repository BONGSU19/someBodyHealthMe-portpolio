...
public class CancelDietRequestAction implements Action {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        // 현재 DIET_COMMENT 상태 확인
        if (dietPlan.getDietComment() != 1) {
            session.setAttribute("message", "등록 요청된 식단만 취소할 수 있습니다.");
            return "redirect:/mydiet/selectCustomDiet.do";
        }

        // dietComment를 0으로 설정하여 등록 요청 취소
        dietPlan.setDietComment(0);

        // dietComment 업데이트
        try {
            dietPlanDAO.updateDietComment(dietId, 0);
            session.setAttribute("message", "식단 등록 요청이 취소되었습니다.");
        } catch (Exception e) {
            e.printStackTrace();
            session.setAttribute("message", "식단 등록 요청 취소 중 오류가 발생하였습니다.");
        }
